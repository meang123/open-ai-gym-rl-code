...
@ray.remote(num_cpus=2,num_gpus=0.125)#(num_cpus=1, num_gpus=0.125,placement_group=pg)
class CostReplayBuffer:
    """
    Replay Buffer.
    """

    def __init__(self, buffer_size, num_sequences, device,image_size=64):
        self.height = image_size
        self.width = image_size

        state_shape=Box(0, 255, (18, self.height, self.width), np.uint8).shape
        ometer_shape=Box(-np.inf, np.inf, shape=(40, 2), dtype=np.float32).shape
        tgt_state_shape=Box(0, 255, (3, self.height, self.width), np.uint8).shape
        ...
        self.action_ = torch.empty(buffer_size, num_sequences, *action_shape,device=device)#,device=device
    ...
    def sample_latent(self, batch_size):
        ...
        state_ = torch.tensor(state_, dtype=torch.uint8,device=self.device).float().div_(255.0)#,device=self.device
