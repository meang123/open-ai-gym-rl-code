...
"""
class PER_Buffer:
    def __init__(self,opt,device ='cuda:0'): # opt is option argument in main file. device for gpu memory setting buffer content to cpu
        self.device = device
    ...
    def sample(self,beta) -> typing.Tuple[np.array, np.array, np.array]:
        ...
        idxs = np.random.choice(np.arange(ps.size),
